...
def _does_file_exist(parameter_name, filepath_input):
    ...
    LOG.debug(
        "Checking that a %s parameter is specified and the file exists", parameter_name
    )
    ...
    # parameter is not provided by the user
        ...
        LOG.error(
            "A %s parameter is not provided in the configuration file", parameter_name
        )
        sys.exit()
...
def _does_path_exist(parameter_name, directory_path_input):
    ...
    try:
        ...
        LOG.error(
            "A %s parameter must be specified in the configuration file", parameter_name
        )
        sys.exit()
...
def check_inputs(
    ...
    # -----------------------------------------------------------------
    if preprocessing_parameters.get(
    ...
    if preprocessing_parameters.get("use_preprocessed_data", None):
        ...
        if preprocessing_parameters.get("preprocess_source_file", False):
            ...
            LOG.error("No preprocessed .npy file specified.")
            LOG.error("Please specify preprocess_source_file in the configuration file")
            quit()

        # ... and check to make sure the source file exists
        _does_file_exist(
            "preprocess_source_file", preprocessing_parameters["preprocess_source_file"]
        )

    # -----------------------------------------------------------------
    # Check that geo_file_path is provided and exists
    # -----------------------------------------------------------------
    _does_file_exist("geo_file_path", supernetwork_parameters["geo_file_path"])
    ...
    if supernetwork_parameters.get("mask_file_path", None):
        _does_file_exist("mask_file_path", supernetwork_parameters["mask_file_path"])
    else:
        LOG.debug(
    ...
    if waterbody_parameters.get("break_network_at_waterbodies", None):
        ...
        LOG.debug("break_network_at_waterbodies == False or is not specified.")
    ...
    if restart_parameters.get("wrf_hydro_channel_restart_file", None):
        ...
        LOG.debug(
            "No channel restart file specified, simulation will begin from a cold start"
        )
    ...
    if qlat_sets:
        if qlat_sets[0].get("qlat_files", None):
            ...
            for s, _ in enumerate(qlat_sets):
                for i, f in enumerate(qlat_sets[s]["qlat_files"]):
                    _does_file_exist(
                        "qlat_files[{}][{}]".format(s, i), qlat_input_folder.joinpath(f)
                    )

        elif qlat_sets[0].get("qlat_input_file", None):
            for s, _ in enumerate(qlat_sets):
                _does_file_exist("qlat_input_file", qlat_sets[s]["qlat_input_file"])
        ...
        _does_path_exist("qlat_input_folder", forcing_parameters["qlat_input_folder"])
    # -----------------------------------------------------------------
    ...
    if streamflow_da:
        if streamflow_da.get("streamflow_nudging", False):
            ...
            _does_file_exist(
                "gage_segID_crosswalk_file", streamflow_da["gage_segID_crosswalk_file"]
            )
            # -----------------------------------------------------------------
            ...
            if streamflow_da.get("wrf_hydro_lastobs_file", None):
                _does_file_exist(
                    "wrf_hydro_lastobs_file", streamflow_da["wrf_hydro_lastobs_file"]
                )
    ...
    if reservoir_da:
        reservoir_persistence_usgs = reservoir_da.get(
            "reservoir_persistence_usgs", False
        )
        reservoir_persistence_usace = reservoir_da.get(
        ...
        if reservoir_persistence_usgs or reservoir_persistence_usace:
            _does_file_exist(
                "gage_lakeID_crosswalk_file", reservoir_da["gage_lakeID_crosswalk_file"]
            )
    # -----------------------------------------------------------------
    ...
    if output_parameters.get("csv_output", None):
        if output_parameters["csv_output"].get("csv_output_folder", None):
            ...
            output_segs = output_parameters["csv_output"].get(
                "csv_output_segments", None
            )
            if not output_segs:
                LOG.debug(
                    "No csv output segments specified. Results for all domain segments will be written"
                )

        else:
            LOG.debug(
                "No csv output folder specified. Results will NOT be written to csv"
            )
    ...
    if output_parameters.get("parquet_output", None):
        if output_parameters["parquet_output"].get("parquet_output_folder", None):
            ...
            output_segs = output_parameters["parquet_output"].get(
                "parquet_output_segments", None
            )
            if not output_segs:
                LOG.debug(
                    "No parquet output segments specified. Results for all domain segments will be written"
                )

        else:
            LOG.debug(
                "No parquet output folder specified. Results will NOT be written in parquet format"
            )
    ...
    if output_parameters.get("chrtout_output", None):
        if output_parameters["chrtout_output"].get("chrtout_read_folder", None):
            ...
            if output_parameters["chrtout_output"].get("chrtout_write_folder", None):
                ...
                LOG.debug(
                    "No chrtout_output folder specified. Defaulting CHRTOUT write location to chrtout_read_folder"
                )

        else:
            LOG.debug(
                "No chrtout_output parameter specified. Results will NOT be written to CHRTOUT"
            )
    else:
        LOG.debug(
            "No chrtout_output parameter specified. Results will NOT be written to CHRTOUT"
    ...
    if output_parameters.get("hydro_rst_output", None):
        if output_parameters["hydro_rst_output"].get(
            "wrf_hydro_channel_restart_source_directory", None
        ):
            _does_path_exist(
                "wrf_hydro_channel_restart_source_directory",
                output_parameters["hydro_rst_output"][
                    "wrf_hydro_channel_restart_source_directory"
                ],
            )

            if output_parameters["hydro_rst_output"].get(
                "wrf_hydro_channel_restart_output_directory", None
            ):
                _does_path_exist(
                ...
                LOG.debug(
                    "No wrf_hydro_channel_restart_output_directory folder specified. Defaulting write location to wrf_hydro_channel_restart_source_directory"
                )

        else:
            LOG.debug(
                "No hydro_rst_output parameter specified. Restart data will NOT be written out"
            )
