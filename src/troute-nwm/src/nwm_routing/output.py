...
def _parquet_output_format_converter(df, start_datetime, dt, configuration, prefix_ids):
    ...
    for i in range(num_time_variables):
        ...
        value_time[start_idx:end_idx] = start_datetime + pd.to_timedelta(
            (timesteps[i] + 1) * dt, unit="s"
        )
...
def nwm_output_generator(
    ...
    if (
        ...
        if wbdyo and not waterbodies_df.empty:
            ...
            i_columns = pd.MultiIndex.from_product([range(nts), ["i"]]).to_flat_index()
            ...
            if not duplicate_ids_df.empty:
                flow_df = flow_df.rename(
                    index=dict(duplicate_ids_df[["synthetic_ids", "lake_id"]].values)
                )
                wbdy = wbdy.rename(
                    index=dict(duplicate_ids_df[["synthetic_ids", "lake_id"]].values)
                )
            ...
            timestep_index = np.where(
                ((np.array(list(set(list(timestep)))) + 1) * dt)
                % (dt * qts_subdivisions)
                == 0
            )
    ...
    if stream_output:
        ...
        if not logFileName == "NONE":
            with open(logFileName, "a") as preRunLog:
                ...
                preRunLog.write("-----\n")
                preRunLog.write(
                    "Output of flow velocity depth files into folder: "
                    + str(Path(stream_output_directory))
                    + "\n"
                )
                preRunLog.write("-----\n")
                ...
                fCalc = int(dt / 60)
                preRunLog.write(
                    "Internal computation of FVD data every "
                    + str(stream_output_internal_frequency)
                    + " minutes\n"
                )
                ...
                preRunLog.write(
                    ...
                    + str(nTimeBins)
                    + " time bins for "
                    + str(len(flowveldepth.index))
                    + " segments per FVD output file\n"
                )
            preRunLog.close()
    ...
    if wbdyo and not waterbodies_df.empty:
        ...
        if not duplicate_ids_df.empty:
            output_waterbodies_df = waterbodies_df.rename(
                index=dict(duplicate_ids_df[["synthetic_ids", "lake_id"]].values)
            )
            output_waterbody_types_df = waterbody_types_df.rename(
                index=dict(duplicate_ids_df[["synthetic_ids", "lake_id"]].values)
            )
        else:
        ...
        if not logFileName == "NONE":
            with open(logFileName, "a") as preRunLog:
                ...
                preRunLog.write("-----\n")
                preRunLog.write(
                    "Output of waterbody files into folder: " + str(wbdyo) + "\n"
                )
                preRunLog.write("-----\n")
            preRunLog.close()

        LOG.debug(
            "writing LAKEOUT files took a total time of %s seconds."
            % (time.time() - start)
        )
            ...
            if len(wrf_hydro_restart_files) > 0:
                ...
                if not logFileName == "NONE":
                    with open(logFileName, "a") as preRunLog:
                        ...
                        preRunLog.write("-----\n")
                        preRunLog.write(
                            "Output of wrf hydro restart files into folder: "
                            + str(Path(wrf_hydro_restart_dir))
                            + "\n"
                        )
                        preRunLog.write("-----\n")
                    preRunLog.close()
    ...
    if chrto:
        ...
        if chrtout_read_folder:
            ...
            if not logFileName == "NONE":
                with open(logFileName, "a") as preRunLog:
                    ...
                    preRunLog.write("-----\n")
                    preRunLog.write(
                        "Output of FVD files into folder: "
                        + str(chrtout_read_folder)
                        + "\n"
                    )
                    preRunLog.write("-----\n")
                preRunLog.close()
        ...
        LOG.debug(
            "writing CHRTOUT files took a total time of %s seconds."
            % (time.time() - start)
        )
    ...
    if csv_output_folder:
        ...
        if return_courant:
            ...
            courant.loc[csv_output_segments].to_csv(
                output_path.joinpath(filename_courant)
            )
    ...
    if parquet_output_folder:
        ...
        timeseries_df = _parquet_output_format_converter(
            flowveldepth,
            restart_parameters.get("start_datetime"),
            dt,
            configuration,
            prefix_ids,
        )

        parquet_output_segments_str = [
            prefix_ids + "-" + str(segment) for segment in parquet_output_segments
        ]
        timeseries_df.loc[
            timeseries_df["location_id"].isin(parquet_output_segments_str)
        ].to_parquet(
        ...
        if return_courant:
            courant = courant.sort_index()
            timeseries_courant = _parquet_output_format_converter(
                courant,
                restart_parameters.get("start_datetime"),
                dt,
                configuration,
            ...
            timeseries_courant.loc[
                timeseries_courant["location_id"].isin(parquet_output_segments_str)
            ].to_parquet(
    ...
    if chano:
        ...
        if isinstance(chano["chanobs_output_directory"], Path):
            chano["chanobs_output_directory"] = (
                str(chano["chanobs_output_directory"]) + "/"
            )
        ...
        if not logFileName == "NONE":
            with open(logFileName, "a") as preRunLog:
                ...
                preRunLog.write("-----\n")
                preRunLog.write(
                    "Output of results at gage locations into folder: "
                    + str(Path(chano["chanobs_output_directory"]) + "\n")
                )
                preRunLog.write("-----\n")
            preRunLog.close()

        LOG.debug(
            "writing flow data to CHANOBS took %s seconds." % (time.time() - start)
        )
        ...
        if nudging_true and lastobs_output_folder and not lastobs_df.empty:
            ...
            if not logFileName == "NONE":
                with open(logFileName, "a") as preRunLog:
                    ...
                    preRunLog.write("-----\n")
                    preRunLog.write(
                        "Output of lastobs data into folder: "
                        + str(lastobs_output_folder)
                        + "\n"
                    )
                    preRunLog.write("-----\n")
                preRunLog.close()
