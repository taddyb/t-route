...
class NHDNetwork(AbstractNetwork):
    ...
    def __init__(
        ...
        self._canadian_gage_link_df = pd.DataFrame(columns=["gages", "link"]).set_index(
            "link"
        )
    ...
    def read_geo_file(
        ...
        self._dataframe = nhd_io.read(
            pathlib.Path(self.supernetwork_parameters["geo_file_path"])
        )
        ...
        synthetic_wb_segments = self.supernetwork_parameters.get(
        ...
        if synthetic_wb_segments:
            ...
            self._dataframe.loc[fix_idx, "key"] = (
                self.dataframe[fix_idx].key + synthetic_wb_id_offset
            ).astype("int64")
        ...
        if self.supernetwork_parameters["mask_file_path"]:
            data_mask = nhd_io.read_mask(
                ...
                layer_string=self.supernetwork_parameters.get(
        ...
        if break_network_at_waterbodies:
            ...
            level_pool_params = self.waterbody_parameters.get(
            ...
            reservoir_da = self.data_assimilation_parameters.get("reservoir_da", {})
