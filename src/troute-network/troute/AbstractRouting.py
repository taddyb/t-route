...
from troute.nhd_network_utilities_v02 import (
    organize_independent_networks,
    build_refac_connections,
)
...
class AbstractRouting(ABC):
    ...
    __slots__ = [
        ...
        "_topobathy_df",
        "_refactored_diffusive_domain",
        "_refactored_diffusive_network_data",
        "_refactored_reaches",
        "_unrefactored_topobathy_df",
        "_all_links",
        "_bad_topobathy_links",
        "_dataframe",
    ]
...
class MCwithDiffusive(AbstractRouting):
    ...
    def update_routing_domain(self, dataframe, connections, waterbody_dataframe):
        ...
        for tw in diffusive_domain:
            headlink_mainstem, rfc_val, rpu_val = list(
                diffusive_domain.get(tw).values()
            )
            if 999999 in headlink_mainstem:
                ...
                links = list(
                    reachable(rconn_diff0, sources=[tw], targets=targets).get(tw)
                )
                ...
                twlink_mainstem = tw
                diffusive_domain_all[twlink_mainstem] = (
                    self.diffusive_domain_by_both_ends_streamid(
                        connections,
                        headlink_mainstem,
                        twlink_mainstem,
                        rfc_val,
        ...
        for tw in self._diffusive_domain:
            ...
            links = list(
                set(self._diffusive_domain[tw]["links"]).intersection(set(links))
            )
            outlet_ids = [connections.get(id) for id in wbody_ids]
            outlet_ids = list(chain.from_iterable(outlet_ids))
            ...
            upstream_boundary_mainstem_link = self._diffusive_domain[tw][
                "upstream_boundary_link_mainstem"
            ]
            ...
            self._diffusive_network_data[tw]["upstream_boundary_link"] = (
            ...
            existing_indicies_in_dataframe = [
                id for id in mainstem_segs if id in dataframe.index
            ]
    ...
    def diffusive_domain_by_both_ends_streamid(
        self, connections, headlink_mainstem, twlink_mainstem, rfc_val, rpu_val
    ):
        ...
        while dslink_mainstem != twlink_mainstem:
            try:
                ...
                LOG.debug(
                    f"KeyError: 'connections' does not have a key '{uslink_mainstem}'"
                )
                return None
        ...
        diffusive_domain = {
            ...
            "rfc": rfc_val,
            "rpu": rpu_val,
            "upstream_boundary_link_mainstem": [headlink_mainstem],
        }
    ...
    def topobathy_df(self):
        if self._topobathy_df.empty:
            ...
            if topobathy_file.suffix == ".nc":
                ...
                self._topobathy_df = read_parquet(topobathy_file, seg_ids).set_index(
                    "hy_id"
                )
                self._topobathy_df.index = self._topobathy_df.index.astype(int)
            ...
            missing_topo_ids = list(
                set(self._all_links).difference(set(topobathy_df.index))
            )
            topo_df_list = []

            for key in missing_topo_ids:
                topo_df_list.append(
                    _fill_in_missing_topo_data(key, self._dataframe, topobathy_df)
                )
            ...
            bad_topobathy_links = list(
                set(missing_topo_ids).difference(set(new_topo_df.index))
            )
...
class MCwithDiffusiveNatlXSectionRefactored(MCwithDiffusive):
    ...
    def topobathy_df(self):
        if self._topobathy_df.empty:
            refactored_topobathy_file = self.hybrid_params.get(
                "refactored_topobathy_domain", None
            )
            self._topobathy_df = read_netcdf(refactored_topobathy_file).set_index(
                "link"
            )
        return self._topobathy_df
    ...
    def refactored_diffusive_domain(self):
        if not self._refactored_diffusive_domain:
            ...
            self._refactored_diffusive_domain = read_diffusive_domain(
            ...
            for tw in self._diffusive_domain:
                ...
                for k in trib_segs:
                    refactored_diffusive_network_data[refac_tw]["connections"][k] = [
                        self._refactored_diffusive_domain[tw]["incoming_tribs"][k]
                    ]
                ...
                refactored_diffusive_network_data[refac_tw]["mainstem_segs"] = (
                    self._refactored_diffusive_domain[tw]["rlinks"]
                )
                refactored_diffusive_network_data[refac_tw][
    ...
    def unrefactored_topobathy_df(self):
        if self._unrefactored_topobathy_df.empty:
            topobathy_file = self.hybrid_params.get("topobathy_domain", None)
            self._unrefactored_topobathy_df = read_netcdf(topobathy_file).set_index(
                "link"
            )
            self._unrefactored_topobathy_df.index = (
                self._unrefactored_topobathy_df.index.astype(int)
            )
        return self._unrefactored_topobathy_df
