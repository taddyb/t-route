...
def build_da_sets(da_params, run_sets, t0):
    ...
    usgs_timeslices_folder = da_params.get("usgs_timeslices_folder", None)
    usace_timeslices_folder = da_params.get("usace_timeslices_folder", None)
    canada_timeslices_folder = da_params.get("canada_timeslices_folder", None)
    LakeOntario_outflow = da_params.get("LakeOntario_outflow", None)
    # User-specified DA ON/OFF preferences
    usace_da = False
    usgs_da = False
    GreatLakes_da = False
    reservoir_persistence_da = da_params.get("reservoir_da", {}).get(
        "reservoir_persistence_da", False
    )
    if reservoir_persistence_da:
        ...
        GreatLakes_da = reservoir_persistence_da.get(
    ...
    # if no user-input timeslice folders, a list of empty dictionaries
        ...
        not usgs_timeslices_folder
        and not usace_timeslices_folder
        and not canada_timeslices_folder
    ):
    ...
    # if user-input timeslice folders are present, build TimeSlice sets
        ...
        for i, set_dict in enumerate(run_sets):
            ...
            if (usgs_timeslices_folder and nudging) or (
                usgs_timeslices_folder and usgs_da
            ):
                filenames_usgs = (
                ...
                filenames_usgs = _check_timeslice_exists(
                    filenames_usgs, usgs_timeslices_folder
                )
            ...
            if usace_timeslices_folder and usace_da:
                ...
                filenames_usace = _check_timeslice_exists(
                    filenames_usace, usace_timeslices_folder
                )
            ...
            if canada_timeslices_folder and GreatLakes_da:
                ...
                filenames_canada = _check_timeslice_exists(
                    filenames_canada, canada_timeslices_folder
                )
                # Add available TimeSlices to da_sets list
...
def build_forcing_sets(forcing_parameters, t0):
    ...
    try:
        ...
        raise TypeError(
            "Aborting simulation because no nexus_input_folder is specified in the forcing_parameters section of the .yaml control file."
        ) from None
    except AssertionError:
        raise AssertionError(
            "Aborting simulation because the nexus_input_folder:",
            qlat_input_folder,
            "does not exist. Please check the the nexus_input_folder variable is correctly entered in the .yaml control file",
        ) from None

    forcing_glob_filter = forcing_parameters.get(
    ...
    if forcing_glob_filter == "nex-*":
        ...
        if not binary_folder:
            ...
            raise (
                RuntimeError(
                    "Output binary qlat folder supplied in config is not empty (already contains '.parquet' files)"
                )
            )
        ...
        nexus_input_folder, forcing_glob_filter = nex_files_to_binary(
            nexus_files_list, binary_folder
        )
    ...
    if run_sets:
        ...
        datetime_list = [t0 + dt_qlat_timedelta * (n) for n in range(nfiles)]
        ...
        while k < len(forcing_filename_list):
            ...
            if k + max_loop_size < len(forcing_filename_list):
                run_sets[j]["nexus_files"] = forcing_filename_list[
                    k : k + max_loop_size
                ]
            else:
            ...
            if nts_accum <= nts:
                run_sets[j]["nts"] = int(
                    len(run_sets[j]["nexus_files"]) * qts_subdivisions
                )
            else:
...
def build_qlateral_array(
    ...
    if nexus_input_folder:
        ...
        if "nexus_files" in run:
            ...
            nexus_file_pattern_filter = run.get("nexus_file_pattern_filter", "*NEXOUT*")
            nexus_files = sorted(nexus_input_folder.glob(nexus_file_pattern_filter))

        qlat_file_index_col = run.get("qlat_file_index_col", "feature_id")
        ...
        qlats_df = pd.concat(
            (
                nexuses_lateralflows_df.loc[int(k)].rename(v)
                for k, v in nexus_to_upstream_flowpath_dict.items()
            ),
            axis=1,
        ).T
        ...
        # zeros.
        all_df = pd.DataFrame(
            np.zeros((len(segment_index), len(qlats_df.columns))),
            index=segment_index,
