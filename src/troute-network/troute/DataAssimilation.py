...
class AbstractDA(ABC):
    ...
    __slots__ = [
        ...
        "_da_parameter_dict",
        "_reservoir_usgs_df",
        "_reservoir_usgs_param_df",
        "_reservoir_usace_df",
        "_reservoir_usace_param_df",
        "_reservoir_rfc_df",
        "_reservoir_rfc_synthetic",
        ...
        "_nStations_usgs",
        "_usgs_Array" "_datesSecondsArray_reservoir_usgs",
...
class NudgingDA(AbstractDA):
    ...
    def __init__(self, network, from_files, value_dict, da_run=[]):
        ...
        if streamflow_da_parameters:
            ...
            da_parameter_dict["diffusive_streamflow_nudging"] = (
                streamflow_da_parameters.get("diffusive_streamflow_nudging", False)
            )
            ...
            if not from_files:
                if legacy_bmi_df:
                    ...
                    if len(usgs_Array) > 0:
                        ...
                        stationStringLengthArray_usgs = value_dict[
                            "stationStringLengthArray_usgs"
                        ]
                        nStations_usgs = value_dict["nStations_usgs"]

                        # Unflatten the arrays
                        df_raw_usgs = a2df._unflatten_array(
                            usgs_Array, nDates_usgs, nStations_usgs
                        )
                ...
                self._usgs_df = (
                    network.link_gage_df.reset_index()
                    .set_index("gages")
                    .join(usgs_df)
                    .set_index("link")
                    .sort_index()
                ...
                if lastobs:
                    ...
                    lastobs_df = pd.DataFrame(
                        ...
                        columns=["gages", "time_since_lastobs", "lastobs_discharge"],
                    ).set_index("gages")
                    ...
                    self._last_obs_df = _reindex_link_to_lake_id(
                        lastobs_df, network.link_lake_crosswalk
                    )
                ...
                if lastobs_crosswalk_file:
                    ...
                    lastobs_file = data_assimilation_parameters.get(
                        "streamflow_da", {}
                    ).get("lastobs_file", False)
                    if lastobs_file:
                        lastobs_df = _read_lastobs_file(lastobs_file)
                        lastobs_df = lastobs_df.set_index("gages")
                        link_gage_df = network.link_gage_df.reset_index().set_index(
                            "gages"
                        )
                        col_name = link_gage_df.columns[0]
                        link_gage_df[col_name] = link_gage_df[col_name].astype(int)
                ...
                if network.link_lake_crosswalk:
                    self._last_obs_df = _reindex_link_to_lake_id(
                        self._last_obs_df, network.link_lake_crosswalk
                    )

                self._usgs_df = _create_usgs_df(
                ...
                if ("canada_timeslice_files" in da_run) & (
                    not network.canadian_gage_df.empty
                ):
                    self._canada_df = _create_canada_df(
                        data_assimilation_parameters,
                        streamflow_da_parameters,
                        run_parameters,
                        network,
        ...
        LOG.debug(
            "NudgingDA class is completed in %s seconds."
    ...
    def update_after_compute(self, run_results, time_increment):
        ...
        """
        streamflow_da_parameters = self._data_assimilation_parameters.get(
            "streamflow_da", None
        )
...
class PersistenceDA(AbstractDA):
    ...
    def __init__(self, network, from_files, value_dict, da_run=[]):
        ...
        reservoir_da_parameters = data_assimilation_parameters.get(
            "reservoir_da", {}
        ).get("reservoir_persistence_da", None)
        streamflow_da_parameters = data_assimilation_parameters.get(
            "streamflow_da", None
        )
        ...
        if not from_files:
            if usgs_persistence:
                if legacy_bmi_df:
                    ...
                    if len(usgs_reservoir_Array) > 0:
                        ...
                        datesSecondsArray_reservoir_usgs = value_dict[
                            "datesSecondsArray_reservoir_usgs"
                        ]
                        nDates_reservoir_usgs = value_dict["nDates_reservoir_usgs"]
                        stationArray_reservoir_usgs = value_dict[
                            "stationArray_reservoir_usgs"
                        ]
                        stationStringLengthArray_reservoir_usgs = value_dict[
                            "stationStringLengthArray_reservoir_usgs"
                        ]
                        nStations_reservoir_usgs = value_dict[
                ...
                reservoir_usgs_df = (
                    network.usgs_lake_gage_crosswalk.reset_index()
            ...
            if usace_persistence:
                if legacy_bmi_df:
                    ...
                    if len(usace_reservoir_Array) > 0:
                        ...
                        datesSecondsArray_reservoir_usace = value_dict[
                            "datesSecondsArray_reservoir_usace"
                        ]
                        nDates_reservoir_usace = value_dict["nDates_reservoir_usace"]
                        stationArray_reservoir_usace = value_dict[
                            "stationArray_reservoir_usace"
                        ]
                        stationStringLengthArray_reservoir_usace = value_dict[
                            "stationStringLengthArray_reservoir_usace"
                        ]
                        nStations_reservoir_usace = value_dict[
                ...
                reservoir_usace_df = (
                    network.usace_lake_gage_crosswalk.reset_index()
            ...
            if usgs_persistence:
                ...
                if not self._usgs_df.empty:
                    gage_lake_df = (
                        network.usgs_lake_gage_crosswalk.reset_index().set_index(
                            ["usgs_gage_id"]
                    ...
                    gage_link_df = (
                        network.link_gage_df["gages"].reset_index().set_index(["gages"])
                    )
                    ...
                    (reservoir_usgs_df, reservoir_usgs_param_df) = _create_reservoir_df(
                        data_assimilation_parameters,
            ...
            if usace_persistence:
                (reservoir_usace_df, reservoir_usace_param_df) = _create_reservoir_df(
                    data_assimilation_parameters,
                    reservoir_da_parameters,
                    streamflow_da_parameters,
                    run_parameters,
        ...
        LOG.debug(
            "PersistenceDA class is completed in %s seconds."
            % (time.time() - PersistenceDA_start_time)
        )

    def update_after_compute(
        self,
        run_results,
    ):
        """
        ...
        self._reservoir_usgs_param_df, self._reservoir_usace_param_df = (
            _set_persistence_reservoir_da_params(run_results)
        )
    ...
    def update_for_next_loop(
        ...
        if not self.usgs_df.empty:
            if reservoir_da_parameters.get("reservoir_persistence_da", {}).get(
                "reservoir_persistence_usgs", False
            ):
                gage_lake_df = (
                    network.usgs_lake_gage_crosswalk.reset_index().set_index(
        ...
        if not network.waterbody_types_dataframe.empty:
            if 2 in network.waterbody_types_dataframe["reservoir_type"].unique():
                if (
                    self.reservoir_usgs_df.empty
                    and len(self.reservoir_usgs_param_df.index) > 0
                ):
            ...
            if 3 in network.waterbody_types_dataframe["reservoir_type"].unique():
                if (
                    self.reservoir_usace_df.empty
                    and len(self.reservoir_usace_param_df.index) > 0
                ):
...
class great_lake(AbstractDA):
    ...
    def __init__(self, network, from_files, value_dict, da_run):
        ...
        data_assimilation_parameters = self._data_assimilation_parameters
        run_parameters = self._run_parameters
        reservoir_persistence_da = data_assimilation_parameters.get(
            "reservoir_da", {}
        ...
        if reservoir_persistence_da:
            greatLake = reservoir_persistence_da.get(
        ...
        LOG.debug(
            "great_lake class is completed in %s seconds."
            % (time.time() - great_lake_start_time)
        )
    ...
    def update_after_compute(self, run_results, time_increment):
        ...
        if tmp_list:
            ...
            great_lakes_param_df["previous_assimilated_time"] = (
                great_lakes_param_df["previous_assimilated_time"] - time_increment
            )
            great_lakes_param_df["update_time"] = (
                great_lakes_param_df["update_time"] - time_increment
            )
...
class RFCDA(AbstractDA):
    ...
    def __init__(self, network, from_files, value_dict):
        ...
        rfc_parameters = self._data_assimilation_parameters.get("reservoir_da", {}).get(
        ...
        if not from_files:
            if rfc:
                if legacy_bmi_df:
                    ...
                    if len(rfc_StationId_array) > 0:
                        ...
                        rfc_StationId_stringLengths = value_dict[
                        ...
                        rfc_df = a2df._bmi_reassemble_rfc_timeseries(
                            rfc_da_timestep,
                            rfc_totalCounts,
                ...
                self._reservoir_rfc_df, self._reservoir_rfc_param_df = (
            ...
            if rfc:
                ...
                lookback_hrs = rfc_parameters.get(
                ...
                self._reservoir_rfc_param_df = pd.DataFrame()
        LOG.debug(
            "RFCDA class is completed in %s seconds." % (time.time() - RFCDA_start_time)
        )
    ...
    def update_after_compute(self, run_results):
        ...
        """
        # get reservoir DA initial parameters for next loop itteration
        self._reservoir_rfc_param_df = _set_rfc_reservoir_da_params(
            self._reservoir_rfc_param_df, run_results
        )
...
def _create_usgs_df(
    ...
    if usgs_files:
        ...
        usgs_df = pd.DataFrame()
    LOG.debug(
        "Reading and preprocessing usgs timeslice files is completed in %s seconds."
        % (time.time() - usgs_df_start_time)
    )
    return usgs_df
...
def _create_LakeOntario_df(run_parameters, t0, da_run):
    ...
    lake_ontario_df.loc[mask, "Date"] = (
        pd.to_datetime(lake_ontario_df.loc[mask, "Date"]) + pd.Timedelta(days=1)
    ).dt.strftime("%Y-%m-%d")

    lake_ontario_df["Hour"] = lake_ontario_df["Hour"].apply(
        lambda x: re.sub(r":\d{2}$", ":00", x)
    )
    # Combine Date and Hour into Datetime
    lake_ontario_df["Datetime"] = pd.to_datetime(
        lake_ontario_df["Date"] + " " + lake_ontario_df["Hour"], format="%Y-%m-%d %H:%M"
    )
    ...
    lake_ontario_df = lake_ontario_df[
        (lake_ontario_df.index >= start_time) & (lake_ontario_df.index <= end_time)
    ]
    ...
    lake_ontario_df["Datetime"] = lake_ontario_df["Datetime"].dt.strftime(
        "%Y-%m-%d_%H:%M:%S"
    )

    LOG.debug(
        "Creating Lake Ontario dataframe is completed in %s seconds."
        % (time.time() - LakeOntario_df_start_time)
    )

    return lake_ontario_df


def _create_canada_df(
    data_assimilation_parameters,
    streamflow_da_parameters,
    run_parameters,
    network,
    ...
    """
    canada_timeslices_folder = data_assimilation_parameters.get(
        "canada_timeslices_folder", None
    )
    # lastobs_file           = streamflow_da_parameters.get("wrf_hydro_lastobs_file", None)
    lastobs_start = data_assimilation_parameters.get(
        "wrf_hydro_lastobs_lead_time_relative_to_simulation_start_time", 0
    ...
    canada_files = [
        canada_timeslices_folder.joinpath(f) for f in da_run["canada_timeslice_files"]
    ]

    if canada_files:
        canada_df = nhd_io.get_obs_from_timeslices(
    ...
    LOG.debug(
        "Reading Canadian timeslice files is completed in %s seconds."
        % (time.time() - canada_df_start_time)
    )
    return canada_df


def _create_reservoir_df(
    data_assimilation_parameters,
    reservoir_da_parameters,
    streamflow_da_parameters,
    run_parameters,
    ...
    """
    res_timeslices_folder = data_assimilation_parameters.get(
        res_source + "_timeslices_folder", None
    )
    ...
    crosswalk_gage_field = streamflow_da_parameters.get(
        "crosswalk_" + res_source + "_gage_field", res_source + "_gage_id"
    )
    crosswalk_lakeID_field = streamflow_da_parameters.get(
        "crosswalk_" + res_source + "_lakeID_field", res_source + "_lake_id"
    )
    qc_threshold = data_assimilation_parameters.get("qc_threshold", 1)
    interpolation_limit = data_assimilation_parameters.get(
        "interpolation_limit_min", 59
    ...
    LOG.info(
        f"Reading {res_source} timeslice files and creating a dataframe is started."
    )
    ...
    res_files = [
        res_timeslices_folder.joinpath(f)
        for f in da_run[res_source + "_timeslice_files"]
    ]
    ...
    if reservoir_df.empty == False:
        reservoir_param_df = pd.DataFrame(
            data=0, index=reservoir_df.index, columns=["update_time"]
        )
        ...
        reservoir_param_df = pd.DataFrame()
    LOG.debug(
        f"Reading {res_source} timeslice files is completed in %s seconds."
        % (time.time() - reservoir_df_start_time)
    )
...
def _set_persistence_reservoir_da_params(run_results):
    ...
    for r in run_results:
        if len(r[4][0]) > 0:
            tmp_usgs = pd.DataFrame(
                data=r[4][1], index=r[4][0], columns=["update_time"]
            )
        ...
        if len(r[5][0]) > 0:
            tmp_usace = pd.DataFrame(
                data=r[5][1], index=r[5][0], columns=["update_time"]
            )
...
def build_lastobs_df(
    ...
    with xr.open_dataset(crosswalk_file) as ds:
        ...
        gage_link_df = pd.DataFrame(data=data_var_dict).set_index(
            [crosswalk_gage_field]
        )
    ...
    LOG.debug(
        f"Building last observation dataframe completed in %s seconds."
        % (time.time() - build_lastobs_start_time)
    )
    return lastobs_df
...
def read_reservoir_parameter_file(
    ...
    """
    LOG.info(
        "Reading and processing reservoir parameter to create crosswalk is started"
    )
    ...
    df1 = df1.reset_index().drop_duplicates(subset="lake_id").set_index("lake_id")
    ...
    LOG.debug(
        f"Reading and processing reservoir is completed in %s seconds."
        % (time.time() - read_reservoir_start_time)
    )
...
def _timeslice_qcqa(
    ...
    observation_df = (
        pd.DataFrame(
            {"stationId": stationId, "datetime": time_str, "discharge": discharge}
        )
    ...
    observation_qual_df = (
        pd.DataFrame(
            {"stationId": stationId, "datetime": time_str, "quality": qual / 100}
        )
        .set_index(["stationId", "datetime"])
    ...
    observation_qual_df = observation_qual_df.mask(
        observation_qual_df < 0, np.nan
    ).mask(observation_qual_df > 1, np.nan)

    # screen-out poor quality flow observations
    observation_df = observation_df.mask(
        observation_qual_df < qc_threshold, np.nan
        ...
        for a, i in enumerate(range(0, len(observation_df_T.columns), step)):
            ...
            jobs.append(
                delayed(_interpolate_one)(
                    observation_df_T.iloc[:, start:stop], interpolation_limit, frequency
                )
            )
...
def _assemble_lastobs_df(
    ...
    df_time = (
        pd.DataFrame(
            data={"discharge": time, "stationIdInd": stationIdInd, "timeInd": timeInd}
        )
        .set_index(["stationIdInd", "timeInd"])
        .unstack(level=1)
    ).to_numpy()
    ...
    for i, idx in enumerate(last_obs_index):
        last_observations.append(df_discharge.iloc[idx, i])
        lastobs_times.append(
            df_time[i, idx]
        )  # .decode('utf-8')  <- TODO:decoding was needed in old version, may need again depending on dtype that is provided by model engine
...
def _rfc_timeseries_qcqa(
    discharge, stationId, synthetic, totalCounts, timestamp, timestep, lake_number, t0
):
    rfc_df = pd.DataFrame(
        {
            "stationId": stationId,
    ...
    for i in validation_index:
        val_lake_number = (
            lake_number  # TODO: placeholder, figure out how to get lake number here...
        )
        val_discharge = validation_df.loc[i].discharge
        val_synthetic = validation_df.loc[i].synthetic

        use_rfc = _validate_RFC_data(
            val_lake_number, val_discharge, val_synthetic, "", "", 300, from_files=False
        )

        use_rfc_df = pd.concat(
            [use_rfc_df, pd.DataFrame({"stationId": [i], "use_rfc": use_rfc})],
            ignore_index=True,
        )

    rfc_df = rfc_df.set_index(["stationId", "datetime"]).unstack(1, fill_value=np.nan)[
        "discharge"
    ]
...
def assemble_rfc_dataframes(
    ...
    new_timeseries_idx = (
        reservoir_rfc_df.columns.get_loc(t0) - 1
    )  # minus 1 so on first call of reservoir_rfc_da(), timeseries_idx will advance 1 position to t0.
    reservoir_rfc_param_df["totalCounts"] = reservoir_rfc_param_df["totalCounts"] + (
        new_timeseries_idx - reservoir_rfc_param_df["timeseries_idx"]
    )
    ...
    reservoir_rfc_param_df["totalCounts"] = reservoir_rfc_param_df[
        "totalCounts"
    ].astype(int)
    reservoir_rfc_param_df["da_timestep"] = reservoir_rfc_param_df[
        "da_timestep"
    ].astype(int)
    reservoir_rfc_param_df["update_time"] = 0
    reservoir_rfc_param_df["rfc_persist_days"] = rfc_parameters.get(
...
def _read_lastobs_file(
    ...
    LOG.debug(
        f"Reading last observation file is completed in %s seconds."
        % (time.time() - read_lastobs_start_time)
    )
    return lastobs_df


def _create_GL_dfs(
    GL_crosswalk_df, data_assimilation_parameters, run_parameters, da_run, t0
):
    # USGS gages:
    usgs_timeslices_folder = data_assimilation_parameters.get(
        "usgs_timeslices_folder", None
    ...
    usgs_GL_crosswalk_df = GL_crosswalk_df[
        GL_crosswalk_df.index.isin([4800002, 4800004])
    ]
    if usgs_files:
        usgs_GL_df = nhd_io.get_GL_obs_from_timeslices(
    ...
    # Canadian gages:
    canadian_timeslices_folder = data_assimilation_parameters.get(
        "canada_timeslices_folder", None
    )
    canadian_files = [
        canadian_timeslices_folder.joinpath(f) for f in da_run["canada_timeslice_files"]
    ]

    canadian_GL_crosswalk_df = GL_crosswalk_df.loc[[4800006]]

    if canadian_files:
        canadian_GL_df = nhd_io.get_GL_obs_from_timeslices(
    ...
    great_lakes_df = (
        pd.concat([usgs_GL_df, canadian_GL_df, lake_ontario_df])
        .rename(columns={"link": "lake_id"})
        .sort_values(by=["lake_id", "Datetime"])
    )

    great_lakes_df["time"] = (
        pd.to_datetime(great_lakes_df["Datetime"], format="%Y-%m-%d_%H:%M:%S") - t0
    )
    great_lakes_df["time"] = great_lakes_df.time.dt.total_seconds().astype(int)
    great_lakes_df.drop("Datetime", axis=1, inplace=True)

    great_lakes_param_df = pd.DataFrame(
        great_lakes_df.lake_id.unique(), columns=["lake_id"]
    ).sort_values("lake_id")
