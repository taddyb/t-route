...
from troute.nhd_network import (
    ...
    reverse_network,
    reachable_network,
    split_at_waterbodies_and_junctions,
    split_at_junction,
    dfs_decomposition,
)
...
class AbstractNetwork(ABC):
    ...
    __slots__ = [
        "_dataframe",
        "_waterbody_connections",
        "_gages",
        "_terminal_codes",
        "_connections",
        "_waterbody_df",
        "_waterbody_types_df",
        "_waterbody_type_specified",
        "_link_gage_df",
        "_canadian_gage_link_df",
        "_independent_networks",
        "_reaches_by_tw",
        "_flowpath_dict",
        "_reverse_network",
        "_q0",
        "_t0",
        "_link_lake_crosswalk",
        "_usgs_lake_gage_crosswalk",
        "_usace_lake_gage_crosswalk",
        "_rfc_lake_gage_crosswalk",
        "_qlateral",
        "_break_segments",
        "_segment_index",
    ...
    def assemble_forcings(
        ...
        qlat_file_index_col = self.forcing_parameters.get(
            "qlat_file_index_col", "feature_id"
        )
        qlat_file_value_col = self.forcing_parameters.get(
        ...
        if self._coastal_boundary_depth_df.empty:
            coastal_boundary_elev_files = self.forcing_parameters.get(
                "coastal_boundary_input_file", None
            )
            coastal_boundary_domain_files = self.hybrid_parameters.get(
            ...
            if coastal_boundary_elev_files:
                ...
                if coastal_boundary_domain_files:
                    coastal_boundary_domain = nhd_io.read_coastal_boundary_domain(
                        str(coastal_boundary_domain_files)
                    )

                    if "coastal_hy_crosswalk" in coastal_boundary_domain.keys():
                        coastal_hy_crosswalk = coastal_boundary_domain[
                            "coastal_hy_crosswalk"
                        ]

                self._coastal_boundary_depth_df = read_coastal_output(
                    coastal_boundary_elev_files, coastal_hy_crosswalk
                )
    ...
    def set_synthetic_wb_segments(self, synthetic_wb_segments, synthetic_wb_id_offset):
        ...
        self._dataframe.loc[fix_idx, "key"] = (
            self._dataframe[fix_idx].key + synthetic_wb_id_offset
        ).astype("int64")
        # Reset key to index
        self.set_index("key")
        self.sort_index()
    ...
    def initialize_routing_scheme(
        ...
        (self._dataframe, self._connections) = routing.update_routing_domain(
            self.dataframe, self.connections, self.waterbody_dataframe
        )

        self._routing = routing
        hyf = (
            self.supernetwork_parameters.get("network_type", None)
            == "HYFeaturesNetwork"
        )
    ...
    def create_independent_networks(
        ...
        if break_network_at_waterbodies:
            wbody_break_segments = wbody_break_segments.union(
                self._waterbody_connections.values()
            )

        if break_network_at_gages:
            gage_break_segments = gage_break_segments.union(self.gages["gages"].keys())

        (self._independent_networks, self._reaches_by_tw, self._reverse_network) = (
        ...
        LOG.debug(
            "reach organization complete in %s seconds." % (time.time() - start_time)
        )
        ...
        if self.break_points["break_network_at_waterbodies"]:
            ...
            if from_files:
                ...
                # if no restart file is provided, default initial states
                    ...
                    waterbodies_initial_states_df["qd0"] = (
                        waterbodies_initial_ds_flow_const
                    )
                    waterbodies_initial_states_df["h0"] = (
                        waterbodies_initial_depth_const
                    )
                ...
                if len(waterbodies_initial_states_df) > 0:
                    waterbodies_initial_states_df = pd.DataFrame(
                        data=waterbodies_initial_states_df.reshape(
                            len(waterbodies_initial_states_ids), -1
                        ),
        ...
        if from_files:
            if restart_parameters.get("lite_channel_restart_file", None):
                ...
                t0_str = nhd_io.get_param_str(
                    restart_parameters["wrf_hydro_channel_restart_file"], "Restart_Time"
                )
    ...
    def build_forcing_sets(
        ...
        try:
            ...
            raise TypeError(
                "Aborting simulation because no qlat_input_folder is specified in the forcing_parameters section of the .yaml control file."
            ) from None
        except AssertionError:
            raise AssertionError(
                "Aborting simulation because the qlat_input_folder:",
                qlat_input_folder,
                "does not exist. Please check the the nexus_input_folder variable is correctly entered in the .yaml control file",
            ) from None
        ...
        if forcing_glob_filter == "nex-*" and binary_folder:
            ...
            if not binary_folder:
                ...
                raise (
                    RuntimeError(
                        "Output binary qlat folder supplied in config does not exist"
                    )
                )

            # Add tnx for backwards compatability
            qlat_files_list = list(qlat_files) + list(
                qlat_input_folder.glob("tnx*.csv")
            )
            # Convert files to binary hourly files, reset nexus input information
            qlat_input_folder, forcing_glob_filter = nex_files_to_binary(
                qlat_files_list, binary_folder
            )
        ...
        if forcing_glob_filter == "nex-*":
            ...
            final_timestamp = (
                pd.read_csv(all_files[0], header=None, index_col=[0]).tail(1).iloc[0, 0]
            )
            final_timestamp = datetime.strptime(
                final_timestamp.strip(), "%Y-%m-%d %H:%M:%S"
            )
        ...
        # TODO: Throw errors if insufficient input data are available
            ...
            datetime_list = [self.t0 + dt_qlat_timedelta * (n) for n in range(nfiles)]
            datetime_list_str = [
                datetime.strftime(d, "%Y%m%d%H%M") for d in datetime_list
            ]

            # list of forcing files
            forcing_filename_list = [
                d_str + forcing_glob_filter[1:] for d_str in datetime_list_str
            ]
            ...
            for f in forcing_filename_list:
                try:
                    ...
                    raise AssertionError(
                        "Aborting simulation because forcing file",
                        J,
                        "cannot be not found.",
                    ) from None
            ...
            while k < len(forcing_filename_list):
                ...
                if k + max_loop_size < len(forcing_filename_list):
                    run_sets[j]["qlat_files"] = forcing_filename_list[
                        k : k + max_loop_size
                    ]
                else:
                ...
                if nts_accum <= nts:
                    run_sets[j]["nts"] = int(
                        len(run_sets[j]["qlat_files"]) * qts_subdivisions
                    )
                else:
                ...
                if forcing_glob_filter == "*.CHRTOUT_DOMAIN1":
                    final_timestamp_str = nhd_io.get_param_str(
                        final_qlat, "model_output_valid_time"
                    )
                elif forcing_glob_filter.startswith("*NEXOUT"):
                    final_timestamp_str = datetime.strptime(
                        final_qlat.name.split("NEXOUT", 1)[0], "%Y%m%d%H%M"
                    ).strftime("%Y-%m-%d_%H:%M:%S")
                else:
                    df = read_file(final_qlat)
                    final_timestamp_str = pd.to_datetime(df.columns[1]).strftime(
                        "%Y-%m-%d_%H:%M:%S"
                    )

                run_sets[j]["final_timestamp"] = datetime.strptime(
                    final_timestamp_str, "%Y-%m-%d_%H:%M:%S"
                )
    ...
    def filter_diffusive_nexus_pts(
        self,
    ):
...
def get_timesteps_from_nex(nexus_files):
    ...
    output_file_timestamps = [
        pd.to_datetime(i).strftime("%Y%m%d%H%M") for i in output_file_timestamps
    ]
...
def rewrite_to_parquet(file_args, binary_folder):
    ...
    df = pd.read_csv(
        f"{binary_folder}/tempfile_{tempfile_id}.csv",
        names=["feature_id", output_file_id],
    )
    df.set_index("feature_id", inplace=True)  # Set feature_id as the index
    df[output_file_id] = df[output_file_id].astype(
        float
    )  # Convert output_file_id column to float64
...
def nex_files_to_binary(nexus_files, binary_folder):
    ...
    partial_rewrite_to_parquet = partial(
        rewrite_to_parquet, binary_folder=binary_folder
    )
...
def read_DFlow_output(ds):
    ...
    df = (
        df.reset_index()[["time", "station_name", "depth"]]
        .set_index(["station_name", "time"])
        .unstack("time", fill_value=np.nan)["depth"]
    )
    return df
...
def read_SCHISM_output(ds, coastal_hy_crosswalk):
    ...
    elevation_df = (
        ds["elevation"]
        .loc[:, list(schism_nodes.keys())]
        .to_dataframe()
        .reset_index()
        .drop("nSCHISM_hgrid_node", axis=1)
    )
    ...
    if len(elevation_df) == 0:
        ...
        df = df.set_index(["link", "time"]).unstack("time", fill_value=np.nan)[
            "waterdepth"
        ]
