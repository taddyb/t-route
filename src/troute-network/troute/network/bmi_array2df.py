...
def _time_retrieve_from_arrays(
    dataFrame, timeBase, datesSecondsArrays, timeAxisName, freqString
):
...
def _stations_retrieve_from_arrays(
    dataFrame, stationsArray, stationStringLengthArray, stationAxisName
):
...
def _bmi_reassemble_rfc_timeseries(
    rfc_da_timestep,
    rfc_totalCounts,
...
def _bmi_reassemble_lite_restart(
    columnArray, columnLengthArray, nCol, indexArray, nIndex, Array
):
...
def _bmi_reassemble_lastObs(
    gageArray, gageStringLengthArray, timeSinceArray, lastDischargeArray
):
    # Create empty dataframe with appropriate column names
