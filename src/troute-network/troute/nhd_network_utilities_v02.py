...
def build_connections(supernetwork_parameters):
    ...
    if "waterbody" in cols:
        wbodies = nhd_network.extract_waterbody_connections(param_df[["waterbody"]])
...
def organize_independent_networks(
    connections, wbody_break_segments, gage_break_segments
):
    """
    ...
    for tw, net in independent_networks.items():
        ...
        # break reaches at gages and junctions
            ...
            path_func = partial(
                nhd_network.split_at_gages_and_junctions, gage_break_segments, net
            )
...
def build_forcing_sets(forcing_parameters, t0):
    ...
    try:
        ...
        raise AssertionError(
            ...
            qlat_input_folder,
            "does not exist. Please check the the qlat_input_folder variable is correctly entered in the .yaml control file",
        ) from None
    ...
    if run_sets:
        ...
        for s, _ in enumerate(run_sets):
            final_chrtout = qlat_input_folder.joinpath(run_sets[s]["qlat_files"][-1])
            final_timestamp_str = nhd_io.get_param_str(
                final_chrtout, "model_output_valid_time"
            )
            run_sets[s]["final_timestamp"] = datetime.strptime(
        ...
        datetime_list = [t0 + dt_qlat_timedelta * (n + 1) for n in range(nfiles)]
        datetime_list_str = [datetime.strftime(d, "%Y%m%d%H%M") for d in datetime_list]

        # list of forcing files
        forcing_filename_list = [
            d_str + ".CHRTOUT_DOMAIN1" for d_str in datetime_list_str
        ]
...
def build_qlateral_array(
    ...
    if qlat_input_folder:
        ...
        gw_bucket_col = forcing_parameters.get(
            "qlat_file_gw_bucket_flux_col", "qBucket"
        )
        terrain_ro_col = forcing_parameters.get(
        ...
        qlat_df = pd.DataFrame(
            np.stack(ql_list).T, index=idx, columns=range(len(qlat_files))
        )
...
def build_data_assimilation_folder(data_assimilation_parameters, run_parameters):
    ...
    if usgs_files:
        usgs_df = nhd_io.get_obs_from_timeslices(
            ...
            data_assimilation_parameters.get(
