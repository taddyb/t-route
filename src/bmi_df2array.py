...
def _bmi_disassemble_rfc_timeseries(dataFrame, timeRef):
    ...
    rfc_synthetic_values = dataFrame["synthetic_values"].to_numpy(
...
def _bmi_disassemble_lastObs(lastobs_df):
    # Column entries that are already float or int:
    timeSinceArray = lastobs_df["time_since_lastobs"].to_numpy(
        dtype=np.float64, copy=True
    )
    lastDischargeArray = lastobs_df["lastobs_discharge"].to_numpy(
        dtype=np.float64, copy=True
    )
