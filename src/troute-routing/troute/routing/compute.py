...
def _prep_da_dataframes(
    ...
    if not usgs_df.empty and not lastobs_df.empty:
        ...
        lastobs_segs = lastobs_df.index.intersection(subnet_segs).to_list()
        lastobs_df_sub = lastobs_df.loc[lastobs_segs]
        usgs_segs = (
            usgs_df.index.intersection(subnet_segs).reindex(lastobs_segs)[0].to_list()
        )
...
def _prep_reservoir_da_dataframes(
    reservoir_usgs_df,
    ...
    * there are many returns, because we are passing explicit arrays to mc_reach cython code
    ...
    if not reservoir_usgs_df.empty:
        ...
        if exclude_segments:
            usgs_wbodies_sub = list(
                set(usgs_wbodies_sub).difference(set(exclude_segments))
            )
        ...
        reservoir_usgs_update_time = (
            reservoir_usgs_param_df["update_time"].loc[usgs_wbodies_sub].to_numpy()
        )
        reservoir_usgs_prev_persisted_flow = (
            reservoir_usgs_param_df["prev_persisted_outflow"]
            .loc[usgs_wbodies_sub]
            .to_numpy()
        )
        reservoir_usgs_persistence_update_time = (
            reservoir_usgs_param_df["persistence_update_time"]
            .loc[usgs_wbodies_sub]
            .to_numpy()
        )
        reservoir_usgs_persistence_index = (
            reservoir_usgs_param_df["persistence_index"]
            .loc[usgs_wbodies_sub]
            .to_numpy()
        )
    else:
    ...
    if not reservoir_usace_df.empty:
        ...
        if exclude_segments:
            usace_wbodies_sub = list(
                set(usace_wbodies_sub).difference(set(exclude_segments))
            )
        ...
        reservoir_usace_update_time = (
            reservoir_usace_param_df["update_time"].loc[usace_wbodies_sub].to_numpy()
        )
        reservoir_usace_prev_persisted_flow = (
            reservoir_usace_param_df["prev_persisted_outflow"]
            .loc[usace_wbodies_sub]
            .to_numpy()
        )
        reservoir_usace_persistence_update_time = (
            reservoir_usace_param_df["persistence_update_time"]
            .loc[usace_wbodies_sub]
            .to_numpy()
        )
        reservoir_usace_persistence_index = (
            reservoir_usace_param_df["persistence_index"]
            .loc[usace_wbodies_sub]
            .to_numpy()
        )
    else:
    ...
    if not reservoir_rfc_df.empty:
        ...
        if exclude_segments:
            rfc_wbodies_sub = list(
                set(rfc_wbodies_sub).difference(set(exclude_segments))
            )
        reservoir_rfc_df_sub = reservoir_rfc_df.loc[rfc_wbodies_sub]
        reservoir_rfc_totalCounts = (
            reservoir_rfc_param_df["totalCounts"].loc[rfc_wbodies_sub].to_numpy()
        )
        reservoir_rfc_file = (
            reservoir_rfc_param_df["file"].loc[rfc_wbodies_sub].to_list()
        )
        reservoir_rfc_use_forecast = (
            reservoir_rfc_param_df["use_rfc"].loc[rfc_wbodies_sub].to_numpy()
        )
        reservoir_rfc_timeseries_idx = (
            reservoir_rfc_param_df["timeseries_idx"].loc[rfc_wbodies_sub].to_numpy()
        )
        reservoir_rfc_update_time = (
            reservoir_rfc_param_df["update_time"].loc[rfc_wbodies_sub].to_numpy()
        )
        reservoir_rfc_da_timestep = (
            reservoir_rfc_param_df["da_timestep"].loc[rfc_wbodies_sub].to_numpy()
        )
        reservoir_rfc_persist_days = (
            reservoir_rfc_param_df["rfc_persist_days"].loc[rfc_wbodies_sub].to_numpy()
        )
    else:
    ...
    if not great_lakes_df.empty:
        ...
        gl_param_df_sub = great_lakes_param_df[
            great_lakes_param_df["lake_id"].isin(gl_wbodies_sub)
        ]
...
def compute_log_mc(
    ...
    with open(fileName, "a") as preRunLog:
        ...
        if "usgs_timeslices_folder" in data_assimilation_parameters.keys():
            preRunLog.write(
                "usgs timeslice folder: "
                + str(data_assimilation_parameters["usgs_timeslices_folder"])
                + "\n"
            )
        if "usace_timeslices_folder" in data_assimilation_parameters.keys():
            preRunLog.write(
                "usace timeslice folder: "
                + str(data_assimilation_parameters["usace_timeslices_folder"])
                + "\n"
            )
        preRunLog.write("-----\n")
        preRunLog.write("Streamflow DA\n")
        if "streamflow_da" in data_assimilation_parameters.keys():
            outPutStr = "Streamflow nudging: " + str(
                data_assimilation_parameters["streamflow_da"]["streamflow_nudging"]
            )
            preRunLog.write(outPutStr + "\n")
            LOG.info(outPutStr)
            outPutStr = "Diffusive streamflow nudging: " + str(
                data_assimilation_parameters["streamflow_da"][
            ...
            preRunLog.write(
                "Lastobs file: "
                + str(data_assimilation_parameters["streamflow_da"]["lastobs_file"])
                + "\n"
            )
            preRunLog.write("-----\n")
            preRunLog.write("Reservoir DA\n")
            outPutStr = "Reservoir persistence USGS: " + str(
                data_assimilation_parameters["reservoir_da"][
                    "reservoir_persistence_da"
                ]["reservoir_persistence_usgs"]
            )
            preRunLog.write(outPutStr + "\n")
            LOG.info(outPutStr)
            outPutStr = "Reservoir persistence USACE: " + str(
                data_assimilation_parameters["reservoir_da"][
                    "reservoir_persistence_da"
            ...
            LOG.info(outPutStr)
            preRunLog.write(
                "Reservoir RFC forecasts: "
                + str(
                    data_assimilation_parameters["reservoir_da"]["reservoir_rfc_da"][
        ...
        preRunLog.write(
            "Number of downstream connections: " + str(len(connections)) + "\n"
        )
        preRunLog.write("Number of upstream connections: " + str(len(rconn)) + "\n")
        preRunLog.write(
            "Number of waterbody connections: " + str(len(wbody_conn)) + "\n"
        )
        preRunLog.write(
            "Number of reaches by tailwater: " + str(len(reaches_bytw)) + "\n"
        )
        preRunLog.write(
            "Number of independent networks: " + str(len(independent_networks)) + "\n"
        )
        preRunLog.write(
            "Number of waterbodies: " + str(len(waterbodies_df.index)) + "\n"
        )
        preRunLog.write(
            "Waterbody type specified: " + str(waterbody_type_specified) + "\n"
        )
        nH20_types = len(waterbody_types_df.value_counts())
        if nH20_types > 0:
            for nH20 in range(nH20_types):
                preRunLog.write(
                    "Type: " + str(waterbody_types_df.value_counts().index[nH20][0])
                )
                preRunLog.write(
                    "   Number of waterbodies: "
                    + str(waterbody_types_df.value_counts().values[nH20])
                    + "\n"
                )
        preRunLog.write("-----\n")
        preRunLog.write("Gages and relations with waterbodies:\n")
        preRunLog.write(
            "Number of USGS gages in network: " + str(len(usgs_df.index)) + "\n"
        )
        preRunLog.write(
            "Number of USGS gage time bins in network: "
            + str(len(usgs_df.columns))
            + "\n"
        )
        preRunLog.write(
            "Lastobs files, number of gages: " + str(len(lastobs_df.index)) + "\n"
        )
        preRunLog.write(
            "Number of USGS gages in waterbodies: "
            + str(len(reservoir_usgs_df.index))
            + "\n"
        )
        preRunLog.write(
            "Number of USACE gages in waterbodies: "
            + str(len(reservoir_usace_df.index))
            + "\n"
        )
        preRunLog.write(
            "Number of RFC gages in waterbodies: "
            + str(len(reservoir_rfc_df.index))
            + "\n"
        )
        preRunLog.write("\n")
...
def compute_log_diff(
    ...
    with open(fileName, "a") as preRunLog:
        ...
        for i_nTw in range(nTw):
            ...
            preRunLog.write(
                "Reverse connections in network: " + str(revConnGraphLength) + "\n"
            )
        ...
        if not topobathy_df.empty:
            ...
            LOG.info(outPutStr)
            preRunLog.write(
                "Number of segment IDs with topobathy profiles: "
                + str(topoTracesUnique)
                + "\n"
            )
            preRunLog.write("-----\n")
        else:
        ...
        if not coastal_boundary_depth_df.empty:
            ...
            preRunLog.write("\n")
            preRunLog.write("-----\n")
            outPutStr = "Number of segments with coastal boundary condition: " + str(
                coastalTraces
            )
            preRunLog.write(outPutStr + "\n")
            LOG.info(outPutStr)
...
def compute_nhd_routing_v02(
    ...
    if parallel_compute_method == "by-subnetwork-jit-clustered":
        ...
        if not subnetwork_list[0] or not subnetwork_list[1]:
            ...
            for order, ordered_subn_dict in subnetworks_only_ordered_jit.items():
                for subn_tw, subnet in ordered_subn_dict.items():
                    ...
                    reaches_ordered_bysubntw[order][subn_tw] = (
            ...
            for order in subnetworks_only_ordered_jit:
                ...
                for twi, (subn_tw, subn_reach_list) in enumerate(
                    ...
                    reaches_ordered_bysubntw_clustered[order][cluster]["segs"].extend(
                        segs
                    )
                    reaches_ordered_bysubntw_clustered[order][cluster][
                        "upstreams"
                    ].update(subnetworks[subn_tw])

                    reaches_ordered_bysubntw_clustered[order][cluster]["tw"].append(
                        subn_tw
                    )
                    reaches_ordered_bysubntw_clustered[order][cluster][
                        "subn_reach_list"
                    ...
                    if (
                        ...
                        twi < len(reaches_ordered_bysubntw[order])
            ...
            subnetworks_only_ordered_jit, reaches_ordered_bysubntw_clustered = (
                copy.deepcopy(subnetwork_list)
            )
                ...
                for cluster, clustered_subns in reaches_ordered_bysubntw_clustered[
                    ...
                    if order < max(subnetworks_only_ordered_jit.keys()):
                        for us_subn_tw in offnetwork_upstreams:
                            ...
                            flowveldepth_interorder[us_subn_tw]["position_index"] = (
                    ...
                    usgs_df_sub, lastobs_df_sub, da_positions_list_byseg = (
                        _prep_da_dataframes(
                            usgs_df,
                            lastobs_df,
                            param_df_sub.index,
                    ...
                    da_positions_list_byreach, da_positions_list_bygage = (
        ...
        if not subnetwork_list[0] or not subnetwork_list[1] or not subnetwork_list[2]:
            ...
            subnetworks_only_ordered_jit, reaches_ordered_bysubntw, subnetworks = (
                subnetwork_list
            )
                ...
                jobs.append(
                    delayed(compute_func)(
                        ...
                        lastobs_df_sub.get(
                            "lastobs_discharge",
                            pd.Series(
                                index=lastobs_df_sub.index, name="Null", dtype="float32"
                            ),
                        ).values.astype("float32"),
                        lastobs_df_sub.get(
                            "time_since_lastobs",
        ...
        for twi, (tw, reach_list) in enumerate(reaches_bytw.items(), 1):
            ...
            for us_subn_tw in offnetwork_upstreams:
                subn_tw_sortposition = param_df_sub.index.get_loc(us_subn_tw)
                flowveldepth_interorder[us_subn_tw]["position_index"] = (
                    subn_tw_sortposition
                )
            ...
            qlat_sub = qlat_sub.reindex(
                np.setdiff1d(param_df_sub.index, offnetwork_upstreams)
            )
            q0_sub = q0_sub.reindex(param_df_sub.index)
    ...
    for tw in diffusive_network_data:  # <------- TODO - by-network parallel loop, here.
        ...
        if tw in coastal_boundary_depth_df.index:
            coastal_boundary_depth_bytw_df = (
                coastal_boundary_depth_df.loc[tw].to_frame().T
            )
        else:
