...
def test_build_connections(test_param_df, test_terminal_code, expected_connections):
    ...
    terminal_codes = terminal_codes | set(
        test_param_df[~test_param_df["downstream"].isin(test_param_df.index)]["downstream"].values
    )
...
def test_extract_waterbodies(test_param_df, test_waterbody_null_code, expected_wbody_connections):
    wbody_connections = nhd_network.extract_waterbody_connections(
        test_param_df, 
        "waterbody", 
        test_waterbody_null_code
    )
