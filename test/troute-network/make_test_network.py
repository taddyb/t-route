...
def make_network_from_segment(flowpaths, edges, attributes, depth, segment):
    ...
    sub_attributes.to_file(
        "hy_network.gpkg", layer="flowpath_attributes", driver="GPKG"
    )
    # make json version
    sub_flowpaths = pd.merge(
        sub_flowpaths, sub_attributes.drop("geometry", axis=1), on="id"
    )
    sub_flowpaths.drop("geometry", axis=1).to_json(
        "flowpath_attributes.json", orient="index", indent=2
    )
    sub_edges.drop("geometry", axis=1).to_json(
        "flowpath_edge_list.json", orient="records", indent=2
    )
...
if __name__ == "__main__":
    ...
    """
    warnings.warn(
        "This script is deprecated. Please see test/ folder domain files for sample networks",
