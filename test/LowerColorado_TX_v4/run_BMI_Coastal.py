...
def run_troute(config):
    ...
    (timeArray_coastal, nTimes_coastal) = df2a._time_from_df(
        coastalDataFrame, coastal_timeRef
    )
    ...
    # USGS dataframe
    troute.set_value(
        "datesSecondsArray_usgs", DAforcing.get_value("datesSecondsArray_usgs")
    )
    ...
    troute.set_value(
        ...
        DAforcing.get_value("stationStringLengthArray_usgs"),
    )
    ...
    troute.set_value(
        ...
        DAforcing.get_value("datesSecondsArray_reservoir_usgs"),
    )
    troute.set_value(
        "nDates_reservoir_usgs", DAforcing.get_value("nDates_reservoir_usgs")
    )
    troute.set_value(
        "stationArray_reservoir_usgs",
        DAforcing.get_value("stationArray_reservoir_usgs"),
    )
    troute.set_value(
        "stationStringLengthArray_reservoir_usgs",
        DAforcing.get_value("stationStringLengthArray_reservoir_usgs"),
    )
    troute.set_value(
        "nStations_reservoir_usgs", DAforcing.get_value("nStations_reservoir_usgs")
    )
    troute.set_value(
        "usgs_reservoir_Array", DAforcing.get_value("usgs_reservoir_Array")
    )

    # USACE reservoir dataframe
    troute.set_value(
        "datesSecondsArray_reservoir_usace",
        DAforcing.get_value("datesSecondsArray_reservoir_usace"),
    )
    troute.set_value(
        "nDates_reservoir_usace", DAforcing.get_value("nDates_reservoir_usace")
    )
    troute.set_value(
        "stationArray_reservoir_usace",
        DAforcing.get_value("stationArray_reservoir_usace"),
    )
    troute.set_value(
        "stationStringLengthArray_reservoir_usace",
        DAforcing.get_value("stationStringLengthArray_reservoir_usace"),
    )
    troute.set_value(
        "nStations_reservoir_usace", DAforcing.get_value("nStations_reservoir_usace")
    )
    troute.set_value(
        "usace_reservoir_Array", DAforcing.get_value("usace_reservoir_Array")
    )
    ...
    troute.set_value(
        "rfc_synthetic_values", DAforcing.get_value("rfc_synthetic_values")
    )
    ...
    troute.set_value(
        ...
        DAforcing.get_value("rfc_StationId_stringLengths"),
    )
    troute.set_value("rfc_List_array", DAforcing.get_value("rfc_List_array"))
    troute.set_value(
        "rfc_List_stringLengths", DAforcing.get_value("rfc_List_stringLengths")
    )
    ...
    timeseries_end = timeseries_start + timedelta(
        hours=config_dict[config]["duration"] - 1
    )
    delta = timedelta(hours=1)
    forcing_files = []
    while timeseries_start <= timeseries_end:
        forcing_files.append(
            base_path
            + "/channel_forcing/"
            + timeseries_start.strftime("%Y%m%d%H%M")
            + ".CHRTOUT_DOMAIN1.csv"
        )
    ...
    for hr in range(config_dict[config]["duration"]):
        ...
        if hr == config_dict[config]["restart"]:
            ...
            DAforcing.set_value(
                "waterbody_df_ids", troute.get_value("waterbody_df_index")
            )
