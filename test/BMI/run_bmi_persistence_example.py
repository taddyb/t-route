...
full_model.set_value(
    "dx",
    np.array(
        [3383.185374, 7247.358765, 2090.045820, 2059.280869, 3909.862245, 1568.900922]
    ),
)
full_model.set_value(
    "n", np.array([0.060000, 0.059635, 0.050000, 0.057287, 0.050000, 0.055400])
)
full_model.set_value(
    "ncc", np.array([0.120000, 0.119270, 0.100000, 0.114575, 0.100000, 0.110800])
)
full_model.set_value(
    "s0", np.array([0.011690, 0.018579, 0.000211, 0.030771, 0.002016, 0.017703])
)
full_model.set_value(
    "bw", np.array([2.860991, 2.809054, 19.379018, 6.677849, 20.134363, 9.857479])
)
full_model.set_value(
    "tw", np.array([4.768318, 4.681756, 32.298364, 11.129749, 33.557271, 16.429131])
)
full_model.set_value(
    "twcc",
    np.array([14.304953, 14.045269, 96.895086, 33.389247, 100.671812, 49.287396]),
)
...
full_model.set_value(
    "cs", np.array([0.585855, 0.610357, 0.251915, 0.581235, 0.247701, 0.519852])
)
...
reservoir_model.set_value("lake_surface__elevation", np.array([-1.000000e09]))
...
for hr in range(120):
    ...
    # Set dynamic values
    full_model.set_value(
        "land_surface_water_source__volume_flow_rate", forcing_vals[hr, :]
    )
    ...
    flowveldepth = pd.DataFrame(
        full_model.get_value("fvd_results").reshape(6, 36),
        index=full_model.get_value("fvd_index"),
    )
    full_fvd = pd.concat([full_fvd, flowveldepth], axis=1)

    # Split network
    # Set dynamic values
    upper_routing_model.set_value(
        "land_surface_water_source__volume_flow_rate", forcing_vals[hr, 0:3]
    )
    ...
    fvd_num_columns = int(
        update_until_duration / upper_routing_model.get_time_step() * 3
    )  # duration of update divided by model time steps times 3 variables (fvd)

    flowveldepth = pd.DataFrame(
        upper_routing_model.get_value("fvd_results").reshape(
            len(fvd_index), fvd_num_columns
        ),
        index=fvd_index,
    )
    ...
    lower_routing_model.set_value(
        "land_surface_water_source__volume_flow_rate", forcing_vals[hr, 3:6]
    )
    ...
    fvd_num_columns = int(
        update_until_duration / lower_routing_model.get_time_step() * 3
    )  # duration of update divided by model time steps times 3 variables (fvd)

    flowveldepth = pd.DataFrame(
        lower_routing_model.get_value("fvd_results").reshape(
            len(fvd_index), fvd_num_columns
        ),
        index=fvd_index,
    )
    ...
    res_fvd = pd.concat(
        [res_fvd, pd.DataFrame(offnetwork_upstream_fvd.reshape(12, 3))], axis=0
    )
